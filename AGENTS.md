...
`dsh-external/plugin-registry` 已转向：**0 patch 薄控制台**——管理官方 repository 插件（`.dsh-plugin`）的浏览器面板。不再含 patch/官方源码改动；交付物是官方 bundle 格式的独立包。
...
- `dsh-external/plugin-registry`：本仓库（薄控制台交付物）。
