...
| 构件 | 内容 | 作用 |
|---|---|---|
| 官方基线 | `dsh2026/test-vlln` 官方快照（snapshot 提交） | 被构建的底座 |
| [`patches/dsh-plugin-registry.patch`](patches/dsh-plugin-registry.patch) | 机制打进官方源码的补丁（CLI 子命令、apiproxy `plugins` 域、client-modules `registerExternal`、tsconfig、组合挂载） | `git apply` 后官方树具备 registry 机制 |
| [`packages/`](packages/) | 插件实现包（`plugin` 核心、`ui-plugin-manager` 管理面板） | 复制进目标 monorepo，随 patch 一起交付 |
| [`examples/`](examples/) | 可运行示例插件（greeter / navbar / task-status / loop） | 验证机制 + 学习模板 |

**集成步骤**（详见 [集成到 dsh](docs/cookbook/integrating-into-dsh.md)）：复制 `packages/` 进 monorepo → `git apply patches/dsh-plugin-registry.patch` → 组合启用 `plugin-local`。
...
- **改机制**：先在 worktree 官方分支提交，再同步本仓库（示例 + 文档 + patch），不要只改本仓库。
...
- `dsh2026/test-vlln`：官方源码仓库（snapshot 基线 + 特性分支），机制件的家。
