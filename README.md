...
DSH 官方自带插件机制，但它是给**产品团队**用的：插件组合随版本固定，安装、启停、管理、分发都不对最终用户开放。想在官方能力之外加一点东西——一个新工具、一段界面增强——用户没有入口。

本插件系统补上这一层：把「写插件」变成「装插件」。清单声明、安装/启停/卸载、Web 管理面板、脚手架与 tarball 分发，全部面向用户；官方已有的插件加一个清单就能进入，不用重写。
