 ...
 * client-modules registerExternal/addRow/removeStyles, the client-graph
 * changed frame, and the tasks/bash peek seam — 26 files after the 49→26
 * slimming, see docs/patch-slimming-design.md); every other mechanism file
 * ships inside the copied distribution packages above. On a different
 * baseline use `git apply --3way` per docs/cookbook/integrating-into-dsh.md.
 *
...
//    Every distribution implementation package ships in packages/ (the
//    plugin + ui-plugin-manager trees below already carry the slimmed-out
//    implementation files: cli.ts, panel-route.ts, the client fetch face);
//    the patch only carries hard official wiring (see patch-slimming-design).
